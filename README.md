...
基于平安证券慧赢风格的期货实时信号看板。Flask 后端 + 单页前端，支持多品种自选、多周期K线、实时信号推送、TDX公式导入。
...
.venv/bin/python server.py          # 访问 http://localhost:8877
```
...
| 多周期K线 | 1/3/5/15/30/60/120分钟、日线、周线，无闪烁增量刷新 |
...
项目语言见 [`CONTEXT.md`](CONTEXT.md)。这个仓库的核心是“公式指标 → 面板展示 → 信号提醒”，不是交易执行系统。
...
| `docs/PROJECT_MAP.md` | 维护视角的端到端项目地图 |
...
`get_latest_signals()` 返回的 `破浪_黄点` / `空仓_绿点` 是主图单项条件；`做多` / `做空` 是叠加 K/D 过滤后的最终提醒信号。
